...
Проект поддерживает развертывание на тестовом и продовом серверах с автоматическим определением окружения.
...
# Убедитесь, что в nginx/conf.d/rit-utils.conf указан правильный домен
...
- Определит окружение (test/prod) по домену в nginx конфиге
- Переключится на нужную ветку Git (test для тестового, main для продового)
- Проверит и настроит SSL сертификаты
...
```bash
# Автоматическое обновление (определяет окружение по домену)
./update.sh

# Или явно указать ветку
./update.sh test   # для тестового окружения
./update.sh main   # для продового окружения
```

**Скрипт `update.sh` автоматически:**
- Определит окружение по домену в nginx конфиге
- Получит изменения из соответствующей ветки Git
- Определит необходимость пересборки (изменения в Dockerfile, зависимостях)
- Перезапустит или пересоберет контейнеры
- Проверит статус и доступность приложения

### Поддержка нескольких окружений

Проект поддерживает работу с тестовым и продовым серверами:
- **Тестовый сервер**: автоматически использует ветку `test`
- **Продовый сервер**: автоматически использует ветку `main`

Окружение определяется автоматически по домену в конфигурации nginx. Подробнее см. [DEPLOYMENT.md](./DEPLOYMENT.md)
...
├── deploy.sh                   # Скрипт первичного развертывания (автоопределение окружения)
├── update.sh                   # Скрипт обновления приложения (автоопределение окружения)
├── DEPLOYMENT.md               # Документация по развертыванию на нескольких серверах
